#ifndef QPORTAUDIOINPUT_H
#define QPORTAUDIOINPUT_H
...
class QPortAudioInput: public QObject, public PortAudio::PortAudioDevStateChangedListener
{
    Q_OBJECT
    ...
    PortAudio::PortAudioDev::Reader r__=[this](const char * buf_, int64_t len_){return write(buf_,len_);};

    void onStateChanged(PortAudio::PortAudioDevInfo::State state_);

    void volumize(char *buf_, int64_t len_);
    int64_t write(const char * buf_, int64_t len_);
    void setState(QPortAudio::State s);

public:
    explicit QPortAudioInput(const QPortAudioDeviceInfo &audioDeviceInfo=QPortAudioDeviceInfo::defaultInputDevice(), const QPortAudioFormat &format_ = QPortAudioFormat(), QObject *parent_ = nullptr);
    ~QPortAudioInput();
    /**
     * @brief format
...
Q_SIGNALS:
    ...
    void notify();
private:
    Q_DISABLE_COPY(QPortAudioInput)
};


#endif // QPORTAUDIOINPUT_H
